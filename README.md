...
Есть два способа. Для новичка проще **способ А** (скопировать файл с компьютера).

### Способ А — скопировать файл `max-zip.js`

1. Скачайте файл **[max-zip.js](./max-zip.js)** из этого репозитория  
   (на GitHub: откройте файл → кнопка **Raw** → правой кнопкой → **Сохранить как…**).
2. Нажмите на значок **Tampermonkey** в браузере.
3. Выберите **«Панель управления»** (Dashboard).
4. Сверху нажмите **«+»** или **«Создать новый скрипт»**.
5. Откроется редактор с шаблоном. **Выделите всё** (`Ctrl+A`) и **удалите**.
6. Откройте сохранённый `max-zip.js` в блокноте, скопируйте **весь** текст (`Ctrl+A`, `Ctrl+C`).
7. Вставьте в редактор Tampermonkey (`Ctrl+V`).
8. Сохраните: **«Файл» → «Сохранить»** или `Ctrl+S`.
9. Закройте вкладку редактора — скрипт уже активен.

### Способ Б — установить по ссылке (если репозиторий на GitHub)
...
2. Tampermonkey покажет окно **«Установка userscript»** → нажмите **«Установить»**.
...
| Файл | Назначение |
|------|------------|
| `max-zip.js` | Сам userscript — его ставят в Tampermonkey |
